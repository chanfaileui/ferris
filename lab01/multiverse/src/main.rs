...
fn get_universe_details(universe_id: u32) -> Option<UniverseDetails> {
    ...
    if universe_id % 3 == 0 {
        return Some(UniverseDetails {
            universe_name: "Milky Way".to_string(),
...
fn main() {
    for id in 1..=15 {
        ...
        if let Some(details) = universe_details {
            println!(
                "Universe with id {0} is called {1}, won by {2} and has a population of {3}",
                id, details.universe_name, details.universe_winner, details.universe_population
            );
        } else {
